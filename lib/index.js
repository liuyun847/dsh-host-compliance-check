/**
 * dsh-host-compliance-check v0.4.1
 *
 ...
 *    (source.kind='plugin:compliance-check' + source.form='notice',summary 一句话;
 *    content 带文件清单与自查清单),
 *    turn 因 next-step 注入而继续,由主模型当场自行判断:
 *      直接收尾 / 简要自查 / 派 subagent 深查(按其结算通知收活;拿到报告后排除误报、
 *      只做适当改进并说明);
 ...
 *
 * 与 0.4.0 的差异(0.4.1):
 *  - 注入正文不再指定"阻塞等待":是否前台等待由会话自身的人格纪律决定,
 *    插件只负责提醒与给出手续(修掉与"永不阻塞"型预设的指令冲突)。
 *
 * 会话格式 v4 兼容(本地修复,未升版本号):
 *  - DSH 0.1.7-rc.1 起会话存储升级到 v4,其准入规则要求 user/message 的
 *    source.kind 是**非空字符串**且**不得等于 'plugin'**(旧版"插件包装"写法已退役);
 *    违反时落盘被拒,整轮以
 *    "format v4 message requires a producer-owned source kind" 失败(错误码 UNKNOWN)。
 *  - 第三方插件注入消息的正确写法是 kind:'plugin:<插件名>',且 source 里**不再有**
 *    plugin 字段 ⇒ 本插件由 { kind:'plugin', plugin:'compliance-check' } 改为
 *    { kind:'plugin:compliance-check' };form/summary 字段与语义不变。
 *
...
export function buildPromptBody(files, requirement) {
  ...
  const parts = [
    ...
    '- 改动较多或存在疑虑:可在本轮结束前派发只读检查子智能体(subagent/subagent_fork),传入本轮需求与上述文件清单,按它的结算通知收活;拿到报告后排除误报(与既定意图无关、基于不完整上下文的意见不必执行),只对确实需要改进的点做适当改进并说明。',
  ]
 ...
 *
 * source.kind 必须取"生产者自有"的取值:会话格式 v4 拒绝 kind === 'plugin'
 * 的旧版插件包装写法(落盘时抛 "format v4 message requires a producer-owned
 * source kind"),第三方插件的正确取值是 'plugin:<插件名>',且 source 里不再有
 * plugin 字段。UI 侧按 source.kind !== 'user' 判为注入行、按 source.form 折叠,
 * 故 kind 改动不影响折叠展示(注入行的标签随之显示为 plugin:compliance-check)。
 */
export function buildNoticeMessage({ content, summary }) {
  const source = {
    kind: 'plugin:compliance-check',
    form: 'notice',
...
/** 组装注入主会话的消息(user 角色,source=plugin:compliance-check notice,模型可见)。 */
...
export function apply(ctx, config) {
  ...
  log('apply: v0.4.1 config=' + JSON.stringify(config ?? {}))
